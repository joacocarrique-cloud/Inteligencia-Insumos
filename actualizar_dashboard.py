...
================================================
Lee Cubo_Oc.aspx desde OneDrive, procesa los datos y genera
un HTML self-contained con todos los datos embebidos.

Uso:
  - Doble click en Windows (busca automáticamente en OneDrive)
  - O desde terminal:
      python actualizar_dashboard.py
      python actualizar_dashboard.py --origen "C:\\ruta\\al\\Cubo_Oc.aspx"
...
try:
    ...
    print("\n" + "=" * 60)
    print("  ERROR: Falta openpyxl (para leer archivos Excel)")
    print("=" * 60)
...
def buscar_archivo(args):
    ...
    if args.onedrive:
        ...
        if not carpeta.exists():
            raise FileNotFoundError(f"Carpeta OneDrive no existe: {carpeta}")
        archivo = carpeta / NOMBRE_ARCHIVO
        if archivo.exists():
            return archivo
        for match in carpeta.rglob(NOMBRE_ARCHIVO):
            return match
        raise FileNotFoundError(f"No se encontró {NOMBRE_ARCHIVO} en {carpeta}")
    ...
    raise FileNotFoundError(
        ...
        f"  Ruta esperada: {ruta_default}\n"
        f"  Usá --origen para especificar la ruta manualmente."
    )
...
def main():
    ...
    parser.add_argument('--origen', help=f'Ruta completa al archivo {NOMBRE_ARCHIVO}')
...
if __name__ == "__main__":
    try:
        main()
        print("")
        input("✓ Terminó correctamente. Presioná Enter para cerrar...")
    except SystemExit:
        pass  # argparse o sys.exit ya manejaron la salida
    except Exception as e:
        print("\n" + "=" * 60)
        print(f"  ✗ ERROR: {e}")
        print("=" * 60)
        print("\n  Si el error persiste, verificá que:")
        print(f"  • El archivo '{NOMBRE_ARCHIVO}' exista en OneDrive")
        print(f"  • La ruta sea correcta: {RUTA_ONEDRIVE_DEFAULT}")
        print("  • Tengas pandas y openpyxl instalados (pip install pandas openpyxl)")
        print("")
